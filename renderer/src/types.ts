declare global {
    interface Window {
        api: {
            ...
            createVacation: (data: { userId: number, user: string, begin: string, end: string }) => { success: boolean, message: string };
            getVacations: (data: { userId: number }) => { success: boolean, message: string, data: [{ id: number, userId: string, begin: string, end: string }] };
            deleteVacation: (data: { id: number }) => { success: boolean, message: string };
        }
    }
}
